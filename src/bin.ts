...
try {
  sardinesConfig = RepositoryClient.setupRepositoryEntriesByConfigFile(sardinesConfigFile)
} catch (e) {
  console.error(e)
  proc.exit(1)
}
...
const main = async() => {
  ...
  writeline(`import { RepositoryClient } from 'sardines-utils'`)
  ...
  // cache drivers
  await cacheDrivers(sardinesConfig!, writeline)
}
